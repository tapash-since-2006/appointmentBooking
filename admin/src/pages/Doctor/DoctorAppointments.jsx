...

        {
          appointments.map((item, index) => (
            <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50' key={index}>
              ...
              {
                item.cancelled
                  ? <p className='text-red-400 text-center font-medium text-md border p-2 border-red-500 rounded-full'>Cancelled</p>
                  : item.isCompleted
                    ? <p className='text-green-500 text-center font-medium text-md border p-2 border-green-500 rounded-full '>Completed</p>
                    : <div className='flex'>
